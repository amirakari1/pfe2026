...
@Component({
  selector: 'app-meal-planner-master',
  templateUrl: './meal-planner-master.component.html',
  styleUrls: ['./meal-planner-master.component.css'],
  animations: [
...
export class MealPlannerMasterComponent {
  score = 0;
  moves = 25;
  gameStarted = false;
  ...
  foodTypes = [
    { type: 'fruit', icon: '🍎', color: '#FF6B6B' },
    { type: 'vegetable', icon: '🥕', color: '#4ECDC4' },
    { type: 'grain', icon: '🍞', color: '#FFE66D' },
    { type: 'protein', icon: '🍗', color: '#95E1D3' }
  ];
  ...
  initializeBoard(): void {
    ...
    for (let row = 0; row < this.boardSize; row++) {
      ...
      for (let col = 0; col < this.boardSize; col++) {
        const randomType = this.foodTypes[Math.floor(Math.random() * this.foodTypes.length)];
        this.board[row][col] = {
  ...
  checkMatches(): void {
    ...
    if (matchedCount > 0 && this.moves > 0) {
      ...
      setTimeout(() => {
        alert(`${this.translate('game.meal.perfect')}! ${this.translate('game.finalScore')}: ${this.score}`);
  ...
  getTileColor(type: string): string {
    const foodType = this.foodTypes.find(f => f.type === type);
    return foodType ? foodType.color : '#ffffff';
  }
