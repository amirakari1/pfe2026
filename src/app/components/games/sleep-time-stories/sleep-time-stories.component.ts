...
@Component({
  selector: 'app-sleep-time-stories',
  templateUrl: './sleep-time-stories.component.html',
  styleUrls: ['./sleep-time-stories.component.css'],
  animations: [
...
export class SleepTimeStoriesComponent {
  score = 0;
  moves = 25;
  gameStarted = false;
  ...
  sleepTypes = [
    { type: 'moon', icon: '🌙', color: '#667eea' },
    { type: 'star', icon: '⭐', color: '#FFD700' },
    { type: 'cloud', icon: '☁️', color: '#E0E0E0' },
    { type: 'bed', icon: '🛏️', color: '#FFB6C1' }
  ];
  ...
  initializeBoard(): void {
    ...
    for (let row = 0; row < this.boardSize; row++) {
      ...
      for (let col = 0; col < this.boardSize; col++) {
        const randomType = this.sleepTypes[Math.floor(Math.random() * this.sleepTypes.length)];
        this.board[row][col] = {
  ...
  checkMatches(): void {
    ...
    if (matchedCount > 0 && this.moves > 0) {
      ...
      setTimeout(() => {
        alert(`${this.translate('game.sleep.dreams')}! ${this.translate('game.finalScore')}: ${this.score}`);
  ...
  getTileColor(type: string): string {
    const sleepType = this.sleepTypes.find(s => s.type === type);
    return sleepType ? sleepType.color : '#ffffff';
  }
