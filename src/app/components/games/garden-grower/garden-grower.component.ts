...
@Component({
  selector: 'app-garden-grower',
  templateUrl: './garden-grower.component.html',
  styleUrls: ['./garden-grower.component.css'],
  animations: [
...
export class GardenGrowerComponent {
  score = 0;
  moves = 25;
  gameStarted = false;
  ...
  vegetableTypes = [
    { type: 'carrot', icon: '🥕', color: '#FFA500' },
    { type: 'tomato', icon: '🍅', color: '#FF6B6B' },
    { type: 'broccoli', icon: '🥦', color: '#4ECDC4' },
    { type: 'pepper', icon: '🫑', color: '#95E1D3' }
  ];
  ...
  initializeBoard(): void {
    ...
    for (let row = 0; row < this.boardSize; row++) {
      ...
      for (let col = 0; col < this.boardSize; col++) {
        const randomType = this.vegetableTypes[Math.floor(Math.random() * this.vegetableTypes.length)];
        this.board[row][col] = {
  ...
  checkMatches(): void {
    ...
    if (matchedCount > 0 && this.moves > 0) {
      ...
      setTimeout(() => {
        alert(`${this.translate('game.garden.harvest')}! ${this.translate('game.finalScore')}: ${this.score}`);
  ...
  getTileColor(type: string): string {
    const vegType = this.vegetableTypes.find(v => v.type === type);
    return vegType ? vegType.color : '#ffffff';
  }
